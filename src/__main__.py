...
async def start(message: Message, user: User):
    ...
    markup = None
    if not next_usage or (datetime.now(ZoneInfo("Europe/Moscow")) + timedelta(seconds=30)) < tz.make_naive(user.next_usage):
        markup = (
...
async def open_box(request: Request):
    ...
    current_datetime = datetime.now(ZoneInfo("Europe/Moscow"))
